...
const WorkHistoryCard = ({ symbol }) => (
  <View style={styles.historyCard}>
    <Text style={styles.historyLabel}>職歴 {symbol}</Text>
    <InputField label="勤務先" placeholder="例：Club ABC" />
    <View style={styles.row}>
      <InputField label="時給" placeholder="例：3,500円" flex={1} />
      <View style={{ width: 10 }} />
      <InputField label="期間" placeholder="例：1年" flex={1} />
    </View>
    <InputField label="退店日" placeholder="例：2024/01" />
    <InputField label="退店理由" placeholder="例：移転のため" multiline />
  </View>
);
...
export default function App() {
  ...
  const [form, setForm] = useState({
    ...
    hireCondition: '', 
    applyMethod: '', applyMethodCustom: '', introducer: '', 
  ...
  const handleViewSubmit = async () => {
    ...
    try {
      ...
      const GAS_URL = "https://script.google.com/macros/s/AKfycbw4W4XK85GF0sEJSDJQxIUB8bj7zBp8AygjHs7HspAf7F_N4GQLKy4IdtK3Pwv3MZ6E1w/exec"; 
