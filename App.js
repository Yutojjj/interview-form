...
const InputField = ({ label, placeholder, multiline = false, flex = 1, keyboardType = 'default', value, onChangeText, error = false, required = false }) => {
  ...
  return (
    <View style={[styles.inputContainer, { flex }]}>
      ...
      <TextInput
        style={[styles.input, multiline && styles.textArea, error && styles.inputError, isFocused && { borderBottomColor: '#76B148', borderBottomWidth: 2 }]}
        placeholder={placeholder} placeholderTextColor="#bbb" multiline={multiline} keyboardType={keyboardType} value={value} onChangeText={onChangeText} selectionColor="#76B148" onFocus={() => setIsFocused(true)} onBlur={() => setIsFocused(false)}
      />
...
const DropdownSelector = ({ label, options, selectedValue, onSelect, error, required, flex = 1 }) => {
  ...
  return (
    <View style={[styles.inputContainer, { flex }]}>
      <View style={styles.labelRow}><Text style={styles.label}>{label}</Text>{required && <Text style={styles.requiredTag}>必須</Text>}</View>
      <TouchableOpacity style={[styles.dropdownTrigger, error && styles.inputError]} onPress={() => setModalVisible(true)}>
        <Text style={[styles.dropdownText, !selectedValue && { color: '#bbb' }]} numberOfLines={1}>{selectedValue || "選択 ▼"}</Text>
      </TouchableOpacity>
        ...
        <View style={styles.modalOverlay}>
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={{ maxHeight: 400 }}>
              {options.map((item) => (
                <TouchableOpacity key={item.toString()} style={[styles.modalItem, selectedValue === item.toString() && { backgroundColor: '#F1F9EE' }]} onPress={() => { onSelect(item.toString()); setModalVisible(false); }}>
...
const SelectButtons = ({ label, options, selectedValue, onSelect, error, required, customBtnStyle }) => (
  <View style={styles.inputContainer}>
    ...
    <View style={[styles.buttonRow, error && { borderBottomWidth: 2, borderBottomColor: '#EF5350', borderRadius: 8 }]}>
      {options.map((opt) => (
        <Pressable key={opt} style={({ pressed }) => [styles.selectBtn, customBtnStyle, selectedValue === opt && styles.selectBtnActive, pressed && styles.selectBtnPressed]} onPress={() => onSelect(opt)}>
  ...
  <View style={styles.inputContainer}>
    ...
    <View style={[styles.buttonRow, error && { borderBottomWidth: 2, borderBottomColor: '#EF5350', borderRadius: 8 }]}>
      {options.map((opt) => {
        ...
        return (
          <Pressable key={opt} style={({ pressed }) => [styles.selectBtn, isActive && styles.selectBtnActive, pressed && styles.selectBtnPressed]} onPress={() => onToggle(opt)}>
...
const WorkHistoryCard = ({ symbol, prefix, data, updateField, isNight = false }) => (
  <View style={[styles.historyCard, isNight && { borderColor: '#76B148', backgroundColor: '#F8FBF8', borderStyle: 'dashed' }]}>
    <Text style={[styles.historyLabel, isNight && { color: '#2E8B57' }]}>{isNight ? `夜職歴 ${symbol}` : `昼職歴 ${symbol}`}</Text>
    ...
    <View style={styles.row}>
      ...
      <View style={{ width: 10 }} /><InputField label="期間" placeholder="例：1年" flex={1} value={data[`${prefix}Period`]} onChangeText={(v) => updateField(`${prefix}Period`, v)} />
    </View>
...
export default function App() {
  ...
  const [form, setForm] = useState({
    ...
    illness: '', illnessDetail: '', shopConditions: '', // 統一
    email: '', pcEmail: '', lineId: '', instaID: '', facebookId: '', xID: '', // 大文字小文字統一
    motivation: '',
    ...
    transportDropoff: '', transportDropoffAddressStatus: '', transportDropoffAddressCustom: '', // 統一
    debt: '', debtDetail: '', transport: '', transportCustom: '', tattoo: '', tattooDetail: '',
    ...
    n1Name: '', n1Type: '', n1Wage: '', n1Period: '', n1QuitDate: '', n1QuitReason: '', // n形式に統一
    n2Name: '', n2Type: '', n2Wage: '', n2Period: '', n2QuitDate: '', n2QuitReason: '',
    n3Name: '', n3Type: '', n3Wage: '', n3Period: '', n3QuitDate: '', n3QuitReason: ''
  });
  ...
  const handleViewSubmit = async () => {
    ...
    const requiredList = [
      ...
      'emName', 'emRelationStatus', 'emPhone', 'emAddressStatus',
      'motivation', 'hireCondition', 'applyMethod', 'daysPerWeek', 'workTime', 'transportDropoff'
    ];
    requiredList.forEach(key => {
      const val = form[key];
      if (!val || (Array.isArray(val) && val.length === 0) || val.toString().trim() === '') { newErrors[key] = true; }
    });
    if (Object.keys(newErrors).length > 0) { setErrors(newErrors); setSubmitError("赤枠の項目が未入力です。スクロールして確認してください。"); return; }
    if (!isAgreed) { setSubmitError("「間違いありませんか？」にチェックを入れてください。"); return; }

    setIsSubmitting(true);
    try {
      const GAS_URL = "https://script.google.com/macros/s/AKfycbxog8LM9Vq74iiLrHur0DefFsiYHP7roXgY2yZ112-uMy9Q3IgXWGPY3p9x2bsYTB7Ffw/exec";
      ...
      {isSent ? (
        <View style={styles.successPage}>
          ...
          <View style={styles.successButtonRow}>
            ...
            <View style={{ width: 15 }} /><TouchableOpacity style={styles.backButton} onPress={handleClose}><Text style={styles.backButtonText}>閉じる</Text></TouchableOpacity>
          </View>
        </View>
      ) : (
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <View style={styles.header}><Text style={styles.headerTitle}>【アルバイト/社員用面接フォーム】</Text><Text style={{ ...fontSettings, color: '#FAFAD2', fontSize: 14 }}>目安:回答時間5分程度</Text></View>
          <ScrollView style={styles.scrollView} contentContainerStyle={styles.content}>
            <Section title="基本情報">
              ...
              <View style={styles.row}>
                ...
                <View style={{ width: 10 }} /><DropdownSelector label="干支(えと)" options={zodiacOptions} required selectedValue={form.zodiac} onSelect={(v) => updateField('zodiac', v)} error={errors.zodiac} flex={1.5} />
              </View>
              ...
              <View style={styles.row}>
                ...
                <View style={{ width: 10 }} /><InputField label="体重(kg)" placeholder="例：50" keyboardType="numeric" value={form.weight} onChangeText={(v) => updateField('weight', v)} flex={1} />
              </View>
            </Section>
            <Section title="緊急連絡先">
              <InputField label="氏名" placeholder="例：山田 太郎" required value={form.emName} onChangeText={(v) => updateField('emName', v)} error={errors.emName} />
              <DropdownSelector label="続柄" options={['父', '母', '兄', '弟', '姉', '妹', '祖父母', 'その他']} required selectedValue={form.emRelationStatus} onSelect={(v) => updateField('emRelationStatus', v)} error={errors.emRelationStatus} />
              {form.emRelationStatus === 'その他' && <InputField label="具体的な続柄" placeholder="例：叔父" value={form.emRelationCustom} onChangeText={(v) => updateField('emRelationCustom', v)} />}
              <InputField label="電話番号" placeholder="例:09012346789" required keyboardType="phone-pad" value={form.emPhone} onChangeText={(v) => updateField('emPhone', v)} error={errors.emPhone} />
              ...
              {form.emAddressStatus === 'その他' && <InputField label="住所の詳細" placeholder="ご実家などの住所" multiline value={form.emAddressCustom} onChangeText={(v) => updateField('emAddressCustom', v)} />}
            </Section>
            <Section title="職業・学歴">
              ...
              {(form.jobDay === 'フリーター/アルバイト' || form.jobDay === '会社員' || form.jobDay === '自営業' || form.jobDay === '昼キャバクラ等') && (
                <View style={styles.dynamicSubSection}>
                  ...
                  <View style={styles.row}>
                    ...
                    <View style={{ width: 10 }} /><InputField label="在籍期間" placeholder="例：2年" flex={1} value={form.currentJobPeriod} onChangeText={(v) => updateField('currentJobPeriod', v)} />
                  </View>
                </View>
              )}
            ...
            <Section title="志望動機・勤務条件">
              ...
              {form.transportDropoff === '希望する' && (
                <View style={styles.dynamicSubSection}>
                  ...
                  {form.transportDropoffAddressStatus === 'その他' && <InputField label="送り先住所の詳細" placeholder="送り先の住所を入力" multiline value={form.transportDropoffAddressCustom} onChangeText={(v) => updateField('transportDropoffAddressCustom', v)} />}
                </View>
              )}
            ...
            <Section title="職歴">
              ...
              {form.nightJobExp === 'ある' && (
                <View style={styles.dynamicSubSection}>
                  ...
                  <WorkHistoryCard symbol="①" prefix="n1" data={form} updateField={updateField} isNight />
                  <WorkHistoryCard symbol="②" prefix="n2" data={form} updateField={updateField} isNight />
                  <WorkHistoryCard symbol="③" prefix="n3" data={form} updateField={updateField} isNight />
                </View>
              )}
            ...
            <Section title="（任意回答）家族・プライベート・SNS・その他">
              ...
              {form.illness === 'ある' && <InputField label="持病の具体的な内容" placeholder="内容" multiline value={form.illnessDetail} onChangeText={(v) => updateField('illnessDetail', v)} />}
              <InputField label="お探しのお店の条件" placeholder="条件" multiline value={form.shopConditions} onChangeText={(v) => updateField('shopConditions', v)} />
              ...
              <InputField label="Instagram ID" placeholder="@insta_id" value={form.instaID} onChangeText={(v) => updateField('instaID', v)} />
            ...
            <View style={styles.consentCardContainer}><View style={styles.consentCard}><Text style={styles.consentText}>入力内容に間違いありませんか？</Text><Switch value={isAgreed} onValueChange={(v) => setIsAgreed(v)} trackColor={{ false: "#767577", true: "#2E8B57" }} /></View></View>
            <Pressable onPress={handleViewSubmit} disabled={isSubmitting} style={({ pressed }) => [styles.submitButton, isSubmitting && styles.submitButtonDisabled, pressed && { backgroundColor: '#007B50' }]}>
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: '#98D96E' }, header: { paddingVertical: 15, alignItems: 'center' }, headerTitle: { ...fontSettings, fontSize: 18, fontWeight: 'bold', color: '#fff' }, scrollView: { flex: 1 }, content: { padding: 16 }, section: { marginBottom: 20, backgroundColor: '#fff', borderRadius: 20, padding: 16, elevation: 4 }, sectionTitle: { ...fontSettings, fontSize: 17, fontWeight: 'bold', color: '#76B148', marginBottom: 4, textAlign: 'center' }, sectionDescription: { ...fontSettings, fontSize: 11, color: '#888', marginBottom: 16, textAlign: 'center' }, inputContainer: { marginBottom: 14 }, labelRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 5 }, label: { ...fontSettings, fontSize: 13, color: '#333', fontWeight: 'bold' }, requiredTag: { ...fontSettings, fontSize: 10, color: '#fff', backgroundColor: '#EF5350', paddingHorizontal: 5, paddingVertical: 2, borderRadius: 4, marginLeft: 8 }, input: { ...fontSettings, backgroundColor: '#F8FBF8', borderRadius: 12, padding: 12, fontSize: 15, color: '#333', borderBottomWidth: 1, borderBottomColor: '#76B148' }, inputError: { borderBottomColor: '#EF5350', borderBottomWidth: 2 }, errorText: { ...fontSettings, color: '#EF5350', fontSize: 11, marginTop: 4 }, textArea: { height: 100, textAlignVertical: 'top' }, row: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }, buttonRow: { flexDirection: 'row', flexWrap: 'wrap' }, selectBtn: { flexGrow: 1, minWidth: '45%', backgroundColor: '#F1F9EE', padding: 12, borderRadius: 25, alignItems: 'center', margin: 4 }, selectBtnActive: { backgroundColor: '#76B148' }, selectBtnPressed: { backgroundColor: '#F2F2F2' }, selectBtnText: { ...fontSettings, fontSize: 12, color: '#76B148', fontWeight: 'bold' }, selectBtnTextActive: { color: '#fff' }, dropdownTrigger: { backgroundColor: '#F1F9EE', borderRadius: 25, padding: 12, alignItems: 'center' }, dropdownText: { ...fontSettings, fontSize: 14, color: '#333' }, modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end' }, modalContent: { backgroundColor: '#fff', borderTopLeftRadius: 30, borderTopRightRadius: 30 }, modalHeader: { padding: 16, borderBottomWidth: 1, borderBottomColor: '#EEE', alignItems: 'center' }, modalTitle: { ...fontSettings, fontSize: 16, fontWeight: 'bold' }, modalItem: { padding: 18, borderBottomWidth: 1, borderBottomColor: '#F9FAFB', flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, modalItemText: { ...fontSettings, fontSize: 16 }, checkmark: { color: '#76B148', fontWeight: 'bold', fontSize: 18 }, dynamicSection: { marginTop: 10, borderTopWidth: 1, borderTopColor: '#EEE', paddingTop: 15, marginBottom: 15 }, dynamicSubSection: { marginTop: 15, padding: 10, backgroundColor: '#F8FBF8', borderRadius: 15, borderLeftWidth: 4, borderLeftColor: '#76B148' }, subSectionTitle: { ...fontSettings, fontSize: 13, color: '#2E8B57', fontWeight: 'bold', marginBottom: 12, textAlign: 'center' }, historySectionLabel: { ...fontSettings, fontSize: 13, color: '#76B148', fontWeight: 'bold', marginTop: 15, marginBottom: 10, textAlign: 'center' }, workTimeNotice: { ...fontSettings, fontSize: 12, color: '#EF5350', fontWeight: 'bold', marginBottom: 8, textAlign: 'center' }, consentCardContainer: { backgroundColor: '#fff', borderRadius: 20, padding: 8, marginBottom: 20, elevation: 4 }, consentCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F1F9EE', padding: 16, borderRadius: 15 }, consentText: { ...fontSettings, flex: 1, fontSize: 13, color: '#333', fontWeight: 'bold' }, submitButton: { backgroundColor: '#76B148', padding: 18, borderRadius: 30, alignItems: 'center', elevation: 2 }, submitButtonDisabled: { backgroundColor: '#CCC' }, submitButtonText: { ...fontSettings, color: '#fff', fontSize: 16, fontWeight: 'bold' }, historyCard: { backgroundColor: '#F9FAFB', borderRadius: 10, padding: 12, marginBottom: 15, borderWidth: 1, borderColor: '#ECEEF1' }, historyLabel: { ...fontSettings, fontSize: 14, fontWeight: 'bold', color: '#2E8B57', marginBottom: 10 }, msgBanner: { marginTop: 15, alignItems: 'center' }, errorTextOnly: { ...fontSettings, color: '#EF5350', fontSize: 14, fontWeight: 'bold', textAlign: 'center' }, successPage: { flex: 1, backgroundColor: '#fff', justifyContent: 'center', alignItems: 'center', padding: 20 }, successLogo: { width: 180, height: 180, marginBottom: 20 }, successTitle: { ...fontSettings, fontSize: 22, fontWeight: 'bold', color: '#76B148', marginBottom: 10 }, successMessage: { ...fontSettings, fontSize: 14, color: '#666', textAlign: 'center', lineHeight: 22, marginBottom: 30 }, successButtonRow: { flexDirection: 'row', justifyContent: 'center', width: '100%' }, backButton: { backgroundColor: '#76B148', paddingVertical: 12, paddingHorizontal: 20, borderRadius: 25, flex: 1, alignItems: 'center' }, backButtonText: { ...fontSettings, color: '#fff', fontWeight: 'bold', fontSize: 14 },
});
